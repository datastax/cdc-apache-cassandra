...
/**
 * DSE 4 variant of {@link AbstractKafkaMutationSender}.
 *
 * <p>Differences from the Cassandra 4 ({@code agent-c4}) version:
 * <ul>
 *   <li><b>Raw {@code ReversedType} cast</b> — DSE 4 ships Cassandra 3.11-era internals where
 *       {@code ReversedType} is not yet generic. The cast in {@link #cqlToAvro} uses the raw
 *       {@code (ReversedType)} form instead of the parameterised {@code (ReversedType<?>)} used in
 *       agent-c4.</li>
 *   <li><b>Avro package relocation</b> — the agent-dse4 shadow JAR relocates
 *       {@code org.apache.avro} → {@code com.datastax.oss.cdc.avro} (see {@code build.gradle})
 *       to avoid clashing with DSE's bundled Avro 1.7.7 at runtime. The source code still
 *       imports the upstream {@code org.apache.avro} symbols; the relocation is transparent at
 *       compile time and applied only during packaging.</li>
 *   <li><b>Agent string in {@link #createCassandraContainer}</b> — integration tests pass
 *       {@code "agent-dse4"} and additionally set {@code DC} / {@code /config} container
 *       overrides required by DSE, which the C4 test does not need.</li>
 * </ul>
 */
@Slf4j
public class KafkaMutationSender extends AbstractKafkaMutationSender<TableMetadata> {

    private static final ImmutableMap<String, org.apache.avro.Schema> avroSchemaTypes =
            ...
            .put(ByteType.instance.asCQL3Type().toString(), org.apache.avro.Schema.create(org.apache.avro.Schema.Type.INT))
            .put(ShortType.instance.asCQL3Type().toString(), org.apache.avro.Schema.create(org.apache.avro.Schema.Type.INT))
    ...
    public Object cqlToAvro(TableMetadata tableMetadata, String columnName, Object value) {
        ...
        // DSE 4 uses a non-generic ReversedType (Cassandra 3.11 internals); use raw cast here.
        // agent-c4 uses the parameterised form: ((ReversedType<?>) columnMetadata.type).baseType
