= Monitor DataStax CDC for Apache Cassandra
...
== Pulsar Source Connector stats

The Pulsar connector framework reports stats for each connector. You can view the stats for a connector like this:
...
The stats *numReceivedFromSource* and *numWritten* indicate how many events have been processed by the source connector. If the connector has errors, the counts are shown. A description of the last seen error is displayed in the *error* field. 
