...
|replication_latency
|The replication latency in milliseconds (the {cdc_cass} processing time minus the {cass-short} mutation writetime).
...
* The https://github.com/datastax/metric-collector-for-apache-cassandra[Metrics Collector for {cass}] with Prometheus and Grafana dashboards provides the same functionality as {dse-short} Metrics Collector, built on the well-supported collectd agent.
