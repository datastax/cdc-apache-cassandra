= Monitor {cdc_cass_first}

== Change Agent Metrics

The change agent is a JVM agent running in Apache Cassandra(R) nodes provides a dedicated MBean type=`CdcProducer` with the following metrics:
