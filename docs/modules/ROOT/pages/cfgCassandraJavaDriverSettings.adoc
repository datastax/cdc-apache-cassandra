== Pass Cassandra Source Connector for Apache Pulsar settings directly to the DataStax Java driver

In your DataStax Cassandra Source Connector for Apache Pulsar configuration file, you can directly pass settings to the DataStax Java driver by using the `datastax-java-driver` prefix.
For example:
...
== Mapping Cassandra Source Connector for Apache Pulsar settings to Java driver properties

The following table identifies functionally equivalent DataStax Cassandra Source Connector for Apache Pulsar and DataStax Java driver settings.

NOTE: If you define both in your configuration, the Source Connector setting take precedence over the `datastax-java-driver.property-name`.
If you do not provide either in your configuration, Source Connector defaults are in effect.
...
There is a difference between the Source Connector's `contactPoints` setting and the Java driver's `datastax-java-driver.basic.contact-points`.
