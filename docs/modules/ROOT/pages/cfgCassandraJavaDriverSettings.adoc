...
In your {csc_pulsar_first} configuration file, you can directly pass settings to the DataStax Java driver by using the `datastax-java-driver` prefix.
For example:
...
The following table identifies functionally equivalent {csc_pulsar} and DataStax Java driver settings.
...
If you do not provide either in your configuration, {csc_pulsar} defaults are in effect.
