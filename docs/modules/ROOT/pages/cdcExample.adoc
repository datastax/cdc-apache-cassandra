...
* Pulsar Cassandra source connector (CSC)
** Pulsar Cassandra Source NAR - use `pulsar-cassandra-source-<version>.nar`
* {company} CDC for Cassandra connector image:https://img.shields.io/github/v/release/datastax/cdc-apache-cassandra?color=green&display_name=tag[link="https://github.com/datastax/cdc-apache-cassandra/releases/latest"]
...
[NOTE]
====
We recommend using the latest CDC agent version (at least version `1.0.4`+) to support Cassandra collection data types.
====
...
----
+
For CDC agent versions *after 1.0.3*, the CDC agent Pulsar connection parameters are provided as system environment variables (see *DSE CDCassandra in the example above).
+
...
. Start Cassandra/DSE and verify your logs are similar to the Cassandra `system.log` file below. `CDC agent started` indicates your CDC agent has started properly.
+
[source,bash]
----
...
. Deploy the Pulsar Cassandra Source Connector (CSC) for each CDC-enabled Cassandra table. The connector consumes from the **events topiCassandra* (where the CDC agent publishes) and writes to the **data topiCassandra* (where downstream consumers read from).
+
[IMPORTANT]
====
