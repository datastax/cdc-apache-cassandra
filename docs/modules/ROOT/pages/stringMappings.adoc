...
In CDC versions *before 1.0.3*, the {cdc_agent} {pulsar-short} connection parameters were provided as extra JVM options after the `.jar` file name in the form of a comma-separated list of `paramName=paramValue`, as below:
...
In CDC versions *after 1.0.3*, the {cdc_agent} {pulsar-short} connection parameters are also provided as system environment parameters in `cassandra-env.sh`. The JVM option above is now appended to `cassandra-env.sh` as below:
...
This document lists the CDC Change Agent parameter mappings between the JVM option strings and {cass-short} strings.
...
| *messagingProvider*
| The streaming platform to publish change events to: `pulsar` (default) or `kafka`. See xref:ROOT:kafka.adoc[].
...
| The {kafka-short} bootstrap servers (comma-separated list of `host:port`). Required when `messagingProvider=kafka`.
...
| The compression type for {kafka-short} messages (`none`, `gzip`, `snappy`, `lz4`, `zstd`).
...
| The {confluent} Schema Registry URL. When set, the agent uses the {confluent} Avro serializer; when unset, registry-less raw Avro is used. See xref:ROOT:kafka.adoc[].
