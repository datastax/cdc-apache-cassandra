...
. Start DSE if it isn't already running:
+
[source,bash]
----
./bin/dse cassandra -f
...
----
+
At this point, everything is set up for the test: A Cassandra table with pre-existing data, a Pulsar topic with a CDC connector, and a Pulsar consumer subscription.
...
In both the Pulsar-admin extension and the standalone Java application, Cassandra related configurations are exclusively passed as command line arguments.
...
Cassandra cluster. If the port isn't specified, it defaults to
