...
This guide explains how to install and configure {cdc_cass_first} for the first time on your Cassandra or DSE cluster and Pulsar cluster.
...
Depending on the workloads of the CDC enabled Cassandra tables, you may need to increase the CPU and memory specification of the Cassandra nodes.
...
And to enable CDC for your Cassandra deployment:
...
The CDC parameter mappings between JVM and Cassandra environment variables are provided in xref:stringMappings.adoc[CDC Environment Parameter Strings]. +
...
For the full set of source configuration options, see xref:install.adoc#datastax-cassandra-source-connector-for-apache-pulsar-settings[{csc_pulsar} settings].
...
For advanced configuration of the Cassandra driver in the {csc_pulsar}, see xref:install.adoc#pass-cdc-for-cassandra-settings-directly-to-the-datastax-java-driver[Pass {csc_pulsar} settings directly to the DataStax Java driver].
...
== Pass {csc_pulsar} settings directly to the DataStax Java driver

In your {csc_pulsar} configuration file, you can directly pass settings to the DataStax Java driver by using the `datastax-java-driver` prefix.
For example:
...
The following table identifies settings that are functionally equivalent between the {csc_pulsar} and the DataStax Java driver.
...
If you do not provide either in your configuration, {csc_pulsar} defaults are applied.
