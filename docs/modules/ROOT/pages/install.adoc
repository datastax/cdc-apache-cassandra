...
The following NAR files are available and please use the version that matches your Pulsar platform.

[cols="1,1"]
|===
| Streaming platform |  NAR file 

| Apache Pulsar 2.8+ | pulsar-cassandra-source-<version>.nar
| Luna Streaming 2.7+ | luna-cassandra-source-<version>.nar
...
To deploy the {csc_pulsar} `NAR` file in your Pulsar cluster, 

**First**, upload the previously extracted NAR file to each of the Pulsar broker nodes at a specific location that Pulsar can access.
As the best practice, it is recommended to put all Pulsar I/O connector NAR files in the *connectors* sub-folder of the Pulsar home directory. 
Create the *connectors* sub-folder if it doesn't exist.

**Second**, for each CDC-enabled C* table, a separate CSC connector needs to be deployed in the Pulsar cluster using the `pulsar-admin sources create` command.

*Note* that for each CDC-enabled table, the C* change agent will send the events to one *events* topic that is automatically created (if Pulsar allows so) with the following naming convention:
...
The following parameters need to be provided in the `pulsar-admin sources create` command.

* Connector name. Please make sure it is unique per CDC-enabled C* table.
* The location of the previously deployed {csc_pulsar} `NAR` file.
...
* Name of the `data` topic the connector will write to.
