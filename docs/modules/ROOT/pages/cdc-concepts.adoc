...
In a typical CDC flow, the Cassandra source connector has multiple instances running in parallel.
...
When a Cassandra cluster has multiple hosts (with multiple commit logs), and they all use the same mutation to calculate the same hash key, then the same consumer always receives it.
...
Initially, it is set to match the Cassandra table's schema as closely as possible, but some data types aren't known in Pulsar (or more accurately, not known in Avro).
...
You can find the Cassandra table's key fields in the key of the record and the change data in the value portion of the record.
