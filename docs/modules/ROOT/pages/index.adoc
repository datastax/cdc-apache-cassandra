...
* {cdc_pulsar}, which is a source connector deployed in your streaming platform
...
. The {cdc_pulsar} processes these events and fetches the mutated Cassandra row.
. The {cdc_pulsar} writes the row into the data topic. This ensures the most recent state of the Cassandra table is replicated into the data topic without the need to time re-order Cassandra mutations.
...
The {cdc_pulsar} updates the schema registry to dynamically reflect the Cassandra table schema.
