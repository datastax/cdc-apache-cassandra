...
1. When CDC is enabled for a given Cassandra table, the event producer running on each node reads the Cassandra commitlog files and emit a message for each Cassandra mutation in a per-table events topic. This message only include the row primary key
2. Then the source connector process these events, it fetches the mutated Cassandra rows through LOCAL_QUORUM read (or LOCAL_ONE if it fails),
3. and writes the row into the data topic. This ensures the most recent state of the Cassandra table is replicated into
the data topic without the need to time re-order Cassandra mutations. Of course, this does not ensure you get all the mutations, but you
get a consistent replication of the data.
...
a memory cache if the LOCAL_QUORUM read succeed through the originator node. Step 2 and 3 are then skipped if the MD5 digest is found in the memory cache. If the LOCAL_QUORUM read through the originator node failed because the node is dead or replaced, the same mutation should arrive from another replicas, eventually after a repair, leading to a consistent replication.
