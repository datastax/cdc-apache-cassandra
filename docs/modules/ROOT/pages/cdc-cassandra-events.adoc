...
Let's create a Cassandra table to illustrate what's happen:
...
When an `INSERT` statement is executed, the CDC agent catch the mutation from the commitlog files and sent a message into the dirty topic. Then the deployed source connector then read the full row from the Cassandra datacenter and publish a message into the clean topic with all regular columns encoded as an AVRO record in the message payload.
...
If you're having issues with CDC events, check the source connector logs on your pulsar function workers. The name of the logs depends on the connectors' name.
...
You can check the https://pulsar.apache.org/docs/en/schema-manage/[Pulsar schema] for the clean topic matches your CQL table:
