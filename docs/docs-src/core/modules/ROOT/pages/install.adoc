...
The CDC parameter mappings between JVM and C* environment variables are provided in xref:string-mappings.adoc[CDC Environment Parameter Strings]. +
For the full set of JVM configuration options, see xref:agent-parameters.adoc[Table Change Agent Parameters].
...
For the full set of source configuration options, see xref:configure-cassandra-source.adoc[{cdc_pulsar} settings].
For the full set of Cassandra authentication options, see xref:configure-cassandra-auth.adoc[Cassandra Authentication settings].
For the full set of Cassandra SSL settings, see xref:configure-cassandra-ssl.adoc[Cassandra SSL/TLS settings].
For advanced configuration of the Cassandra driver in the {cdc_pulsar}, see xref:configure-cassandra-java-driver-settings.adoc[Pass {cdc_pulsar} settings directly to the DataStax Java driver].
...
include::configure-cassandra-source.adoc[]

include::configure-cassandra-auth.adoc[]

include::configure-cassandra-ssl.adoc[]

include::configure-cassandra-java-driver-settings.adoc[]
